...
public class XSRFProtectionFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		LOG.debug("[XSRFProtectionFilter::dofilter()] BEGIN PATH = " + path);
		...
		if (!isDisabled) {
			...
			if (!isRestRequest(hreq)) {
				...
				if (!isXSRFSessionAlive(session)) {
					...
					try {
						...
						LOG.debug(
								"[XSRFProtectionFilter::dofilter()] new XSRF token created and added to session and response - newToken: " + newToken);						
			...
			LOG.debug("[XSRFProtectionFilter::dofilter()] XSRF isDisabled");
		}
		setNoCache(hresp);
